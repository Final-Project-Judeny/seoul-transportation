...
def fetch_and_upload_cultural_facilities(bucket_name, object_name, **kwargs):
    ...
    # S3에 업로드
    s3_path = f"tour/cultural_facilities/수도권_문화시설_정보_{current_time}.json"

    s3_hook = S3Hook(aws_conn_id="aws_conn_id")
    s3_hook.load_string(
        string_data=json_data, key=s3_path, bucket_name=bucket_name, replace=True
    )
